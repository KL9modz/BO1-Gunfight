...
- [plutonium-stats-are-namespaced-per-mod](plutonium-stats-are-namespaced-per-mod.md) — the stats profile is keyed to `fs_game`, so a mod is its own level-1 ladder. ⚠ CORRECTED: there IS a server-side opt-out, the engine dvar `modStats` (GF ships `0` = share the real BO1 rank profile). Stats only — `config_mp.cfg` settings still namespace per mod. A mod-folder RENAME still resets everyone.
...
- [league-dvar-probe-2011-rulesets](league-dvar-probe-2011-rulesets.md) — live probe of BOCL's 2011 ESL/CEVO/ClanBase dvars: 7 engine-registered + 11 string tweakables, all at defaults except OUR deliberate sv_disableClientConsole 1 — which makes player console advice MAIN-MENU-ONLY and menus the only in-game client-exec channel. scr_game_perks 0 flagged as a loadout-killing hazard.
- [bocl-readyup-timeout-execmenu-designs](bocl-readyup-timeout-execmenu-designs.md) — worked designs from the BOCL dive: lobby ready-up (use-poll + statusicon scoreboard mirror), rationed timeouts as a one-round PREMATCH EXTENSION (never a mid-round freeze), lobby all-talk (SetMatchTalkFlag, stock-proven), and the menu-execNow client-console probe (opt-in only).
...
- [custom-weapon-modff-pipeline](custom-weapon-modff-pipeline.md) — custom weapon defs WORK. The linker bakes models/**xanims**/FX transitively from one `weapon,mp/<name>` entry (mod.ff 22KB→178KB) and only image PIXELS are gated. `ERROR: image ... is missing` for stock assets is expected and exits 0 — build_ff's old `^ERROR:` check was FAILING good builds and silently skipping the mod.ff link. Also: cleanup now runs in `finally` (a failed build used to leave staged files shadowing the stock game), `mp/` in the weapon path is mandatory, and `fx_molotov_exp` CRASHES the linker on its scorch-mark decal.
...
- [vps-deploy-repo-path-and-ssh-invocation](vps-deploy-repo-path-and-ssh-invocation.md) — deploy.ps1 lives in `C:\gfdeploy\BO1-Gunfight`. Over SSH it MUST go through cmd.exe (PS 5.1 turns git's stderr into a terminating error). Game process = `plutonium-bootstrapper-win32.exe`.
  ⚠ The clone can COMMIT but never PUSH (`wincredman` needs a TTY), so a box-side commit hard-fails the next deploy's `git pull --ff-only`. Recover by fetching it DOWN: `git fetch "gf-vps:C:/gfdeploy/BO1-Gunfight" main` on the laptop, then push from there.
...
- [custom-camos-bocl-architecture](custom-camos-bocl-architecture.md) — ⚠ **corrects this repo**: BOCL does NOT do camos via `weaponOptions.csv` rows 17+/`CalcWeaponOptions` — its camo dvars clamp to 0..15 and custom camos are **separate weapons** (`camo1_ak47_mp`), forking a weapon file + viewmodel xmodel + material + a per-gun repainted `.iwi` (50/61/55/~30 of them). Stock camos are ONE shared material across all guns; BOCL's are per-gun repaints — separable costs, so the cheap "just add a camo index" half was never tried. ✅ 2026-08-16 the cheap half WORKS and ships: the engine accepts camo indices **>15** (ours run 17-46), a camo is ONE TILING IMAGE plugged in as `colorDetailMap` (not a material — a row naming a material rendered WHITE, so no 3-map camo can live at a new index), and a **carrier material is mandatory** or every camo is flat white. Delivery: `.iwd` of `images/*.iwi`, **Deflate not stored**. Also `CalcWeaponOptions` takes **5** args (clantag/emblem).
- [encore-v8-feature-port](encore-v8-feature-port.md) — the full accounting of CabCon's EnCoRe V8.3 pack (725 options → ~120 distinct mechanics) with a per-feature verdict: ported / already-had / DEAD on dedicated / not-applicable-and-why. ⚠ DEAD means dedicated only — the same features work for the HOST on a listen server. Read this before "adding a mod from another menu"; the answer is probably already here.
