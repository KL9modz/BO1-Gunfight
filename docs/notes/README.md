...
- [sv-hostname-is-discord-rich-presence](sv-hostname-is-discord-rich-presence.md) — sv_hostname is what Plutonium publishes to every player's DISCORD. Unset ⇒ everyone reads BO1's default "BlackOpsPrivate". Plain text only; codes above `^;` leak literally. Includes the real T5 colour table (orange = `^<`).
