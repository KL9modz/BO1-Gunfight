...
- [hudelem-fadeovertime-lerps-color](hudelem-fadeovertime-lerps-color.md) — `fadeOverTime` arms an interpolation over the whole RGBA, not just alpha, so a `.color` write lands as a CROSSFADE if a fade is still in flight (white→red hitmarker read as PINK). ⚠ `fadeOverTime(0)` is NOT a snap (tried, failed live; stock passes 0 nowhere in raw/maps/mp) — give the write a REAL 0.05s window, re-snap a frame later because stock re-arms its 1s fade in the same frame, and re-arm stock's flash after. An intermittent wrong-colour bug that depends on RECENCY is this, not two overlapping elements.
