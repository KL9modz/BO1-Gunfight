...
- [claude-rc-restart-orphans-every-session](claude-rc-restart-orphans-every-session.md) — a session is a CHILD of the `claude rc` server, so a restart makes every open session unresumable ("Environment deleted"); why GF-ClaudeRC can never use the run_service flight recorder (it reparents, and RC needs svchost), and what watchdog check 1d does instead.
