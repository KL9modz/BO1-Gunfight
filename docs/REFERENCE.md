...
#### `gf_camoPool()` / `gf_rollCamo()`
**The camo rotation, and `gf_camoPool` is the edit surface** — it returns the curated array of camo indices the `-1` (Random) loadouts may roll, as an explicit commented list where uncommenting a line adds a camo and commenting one out drops it. `gf_rollCamo()` is a single `randomInt` draw from it. Called only from `gf_load` at **pool build** (53 loadouts × 2 slots, once per match — never per spawn, never per frame), so rebuilding the small array on each call is free.
