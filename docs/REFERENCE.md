...
- **small** (default below 9 seated humans): curated clustered spawns from `_gf_locations.gsc` (falling back to `mp_wager_spawn`, then `mp_tdm_spawn`); the baked wager blockers (`gun`/`oic`/`hlnd`/`shrp`) are kept in the `_gameobjects` allow-list to shrink the play space; the wager compass material is applied; overtime uses the curated flag spot.
- **large** (auto at **9+ seated HUMANS** — a 5v4 human split or more; bots NEVER trigger it, so a bot-padded 6v6 stays small): the full-map `mp_tdm_spawn` pool; wager blockers are omitted so `_gameobjects::main` deletes them; overtime uses the native Domination B flag (`dom` is always kept in the allow-list so the flag survives); large mode also overrides `level.timeLimit` with `scr_gf_timelimit_large`.

`auto` cannot trust a live roster count inside `onStartGameType` because bots and late joiners connect after it (`_bot::init` is threaded at its end). It therefore reads `game["gf_autoLargeMode"]`, captured once the round is active and everyone has spawned by `gf_updateAutoTeamMode` (called from `gf_tryActivateRound`, counting via `gf_countSeatedHumans`/`gf_autoLargeFromHumans`) and persisted across `map_restart` in `game[]`; a live human count is only a first-setup fallback. Each mode reads its own `_large`-suffixed copy of the tunables (round length, overtime limit, capture time) so flipping modes never clobbers the other's value. The mode branches live in `onStartGameType` (spawns, allow-list, wager assets), `onSpawnPlayer` (curated vs start spawns), and `gf_getOvertimeFlagTrigger`. The health panel's skulls-vs-`Alive: N` readout is a separate, per-team body-count (> 4) decision in `_gf_hud` — it no longer shares a switch point with the spawn mode.
...
Delivery happens through the `level.giveCustomLoadout` hook (`gf_giveCustomLoadout`, called by `_class::giveLoadout`): it blanks the player (`setupBlankRandomPlayer`), gives primary/secondary/knife with packed camo options, gives lethal (Tomahawks get 2) and tactical with clamped clip counts, gives the placed equipment (skipped for bots), grants the fixed base perks (Lightweight, Marathon, Flak Jacket plus pros), applies admin RCON perk overrides (`gf_perk_on`/`gf_perk_off` via `gf_applyPerkList`), and threads the loadout HUD. Each loadout is DEALT two camos after the pool shuffle (`load["camo"]`, `load["camoSecondary"]`, from the curated `gf_camoPool()` list — stock 0-15 plus this mod's own 17-46; `gf_dealCamos` walks the shuffled pool in play order dealing from two shuffled bags, so every enabled camo is handed out once before any of them repeats), packed by `CalcWeaponOptions` and passed as `GiveWeapon`'s 3rd arg; special primaries (Minigun/M202, precached via `PrecacheItem` in `onPrecacheGameType`) force camo 0. Key vars: `level.gf_currentLoad`, `game["gf_pool"]`, `level.gf_cfg_roundsPerLoadout`.
...
- **Loadout overview** — `gf_showWeaponHUD` pushes 8 icon materials + 8 names + the anchor (`ui_gf_lo_*`) for a create-a-class-style summary (primary, secondary, lethal, tactical, equipment, 3 perks), holds ~8s, then slides out (`gf_slideLoadout`). The intro snaps; only the outro animates.
...
- **Score popup** — `gf_showScorePopup` renders on its OWN element (`self.gf_popupElem`, a `NewScoreHudElem` from a render-cap-exempt pool; the stock `hud_rankscroreupdate` is parked offscreen each spawn) to show "+1 Kill"/"+1 Assist" in the stock yellow style, with priority so an Assist can't stomp a kill. Text comes from `GF_POPUP_ELIMINATION`/`GF_POPUP_ASSIST` in `localizedstrings/gf.str` — the reference names are historical; only the strings changed.
...
`onSpawnPlayer` (`gf.gsc`) resolves the actual spawn point: in small mode it first asks `gf_getCustomSpawnPoint` (`_gf_locations.gsc`) for a curated point; on a curated miss it falls back to the map's own `mp_wager_spawn` pool via telefrag-aware `getSpawnpoint_NearTeam` (in-bounds of the wager play area by construction, biased toward the teammates already on the curated points — small mode never spawns anyone outside the baked blockers), and only a map with no wager pool (which also has no blockers) degrades to the team-specific start spawns (deliberately not `getSpawnpoint_NearTeam` on the shared start pool, which could place a late bot on the wrong side). Curated spawns are defined per map in `gf_getCustomSpawnLocations` (per-team origin/yaw lists, built via `gf_spawnSet`/`gf_sp`) and per-map overtime flag points in `gf_getCustomOvertimeLocation` (`gf_ot`, with capture radius/height). `gf_initCustomLocations` loads these into `level.gf_customSpawns`/`level.gf_customOvertimeLocation`, normalizes and validates them (a set is dropped unless both teams have at least one point). `gf_getCustomSpawnPoint` cycles a per-team cursor through the round's set so successive spawners fan out across the cluster, resetting the cursor each round. Maps with no curated entry (e.g. `mp_firingrange`) fall through to the big-map defaults intentionally. Spawn FX/influencers use the stock `_spawnlogic`/`_spawning` paths (`updateAllSpawnPoints`, `create_map_placed_influencers`).
...
|---|---|---|---|---|
| `scr_gf_timelimit` | `0.7` | `0`–`1440` (min) | small | Round length in minutes for small mode. `0.7` = 42s. |
...
| `scr_gf_lethals` | `1` | `0` \| `1` | both | `0` = no lethal grenade is given to anyone, on any loadout (`gf_giveCustomLoadout` skips the give **and** the `SwitchToOffhand`); the overview's lethal slot is hidden (`ui_gf_lo_show2`). Read live at each spawn (`gf_slotOn`), so it lands on the next spawn — the next round for anyone already alive. No pool rebuild: the loadout still carries its lethal, so switching back on restores exactly the same grenades. Empty/unset reads as `1`. |
| `scr_gf_tacticals` | `1` | `0` \| `1` | both | Same, for the tactical slot (`ui_gf_lo_show3`). |
| `scr_gf_equipment` | `1` | `0` \| `1` | both | Same, for the placed-equipment slot (`ui_gf_lo_show4`). Independent of a loadout's own `"none"` equipment (`gf_slotEmpty`) and of the existing bot exclusion — either of those still hides/skips the slot with this at `1`. |
| `scr_gf_camo_base` | `1` | `0` \| `1` | both | `0` = the rotation may not roll any **stock** camo (1-14 Dusty…Flora, plus Gold if the modded switch is also off). Empty/unset reads as `1`. |
| `scr_gf_camo_modded` | `1` | `0` \| `1` | both | `0` = the rotation may not roll any of **this mod's** camos (17-55). Empty/unset reads as `1`. ⚠ **Gold (15) belongs to BOTH sets** — it survives either switch and is gone only when both are off, which leaves every gun on its plain factory finish (index `0`, never a camo and always allowed — which is also why the Minigun/M202/Finger-Gun pins are untouched). Both switches are read at **pool build** (`gf_camoPool` filters through `gf_camoAllowed`) **and** at each spawn's give (`gf_resolveCamo`), so a flip lands on the next spawn rather than the next match; a loadout already carrying a now-disabled camo is remapped **deterministically** (`idx % enabledSet.size`), never re-rolled, so every client still resolves the same gun. `gf_force_camo` overrides both (an explicit admin force wins). Panel: DASHBOARD → GUNFIGHT → *Camo*. |
| `scr_gf_headshot_scale` | `0.8` | `0`–`3` | both | Multiplier on **final** headshot damage. The engine applies the weapon file's baked hit-location multiplier before GSC sees the damage (`MOD_HEAD_SHOT` is set at `_globallogic_player.gsc:731-738`, before the `level.onPlayerDamage` hook at `:741`, whose return value `:742-743` takes as the applied damage) — so this rescales that result globally (`1` = stock, `0.5` = half, `0` = headshots void; a positive scale never rounds a hit below 1 damage); it cannot set a weapon's multiplier to an absolute value. Applied at the top of `gf_onPlayerDamage` so the scaled value flows through the damage score, the most-HP time-out decision, and the red kill-marker test (FF headshots scale too — the engine applies FF damage even though it is never scored). Read live per hit (`gf_headshotScale`), so an RCON `set` lands on the next shot; registered per round from `gf.gsc::gf_roundApplyTuning` for the panel sweep. Interacts with Body Armor: everyone takes −20% on non-headshot bullets, so at `1` headshots are worth proportionally more than the weapon files say — the `0.8` default cancels that premium exactly, restoring the stock head:body ratio (the overall softer TTK stays); raise toward `1` to bring the premium back. ⚠ Does **not** affect the sniper one-shot: the snipers' 1.5× zone covers neck/upper chest too (not headshots), and sniper rounds drop Body Armor from everyone. |
...
| `gf_capture_time` | `3.5` | `0.5`–`60` (s) | small | OT zone hold-to-capture time, small mode. |
| `gf_capture_time_large` | `5` | `0.5`–`60` (s) | large | OT zone hold-to-capture time, large mode. |
| `scr_gf_teamspawnmode` | `auto` | `auto` \| `large` \| `small` | both | Spatial mode selector. `auto` goes large at **9+ seated humans** (a 5v4 human split or more, `gf_autoLargeFromHumans`); bots never trigger it. An invalid value is rewritten to `auto`. `large`/`small` pin the mode. |
| ~~`scr_gf_largemode_minplayers`~~ | *(retired)* | — | — | **RETIRED — no longer read.** The split is hard-wired to the 9-human threshold in `gf_autoLargeFromHumans()`. Any stale cfg value is inert. Pin the mode with `scr_gf_teamspawnmode`. |
| `scr_team_maxsize` | `0` (shipped cfg sets `6`) | `>0` caps team | both | If `>0`, caps players per team; overflow is redirected to spectator on spawn (`gf_playerSpawnedCB`). Shipped `dedicated.cfg` sets `7` (up to 7v7); with `sv_maxclients` 18 that's 14 playing + 1 democlient (`demo_enabled 1`) + 3 spectator. Set `4` for a 4v4-capped server. **This is the "force spectator for everyone else" lever** — not `gf_team_lock`, which instead makes `gf_fill_n` the human cap and would force bots to pad to that size every round. |
| `scr_gf_match_prematch_seconds` | `20` | `2`–`30` (s) | both | Native prematch countdown length for the match's first round (longer intro). |
...
#### `gf_getCaptureTime()`
Returns the OT zone hold-to-capture time in seconds, mode-aware: `gf_capture_time_large` (default 5, clamp 0.5–60) in large mode, else `gf_capture_time` (default 3.5, clamp 0.5–60). Reads via `gf_cfgFloat`.
...
Announces overtime entry. Plays `mpl_hq_cap_us` to all players, fires the stock `leaderDialog("overtime")` callout, threads `gf_overtimeCueVO()` for the CTF cue behind it, and shows a 5.0s red `oldNotifyMessage` "OVERTIME" title to every player (using `game["strings"]["overtime"]` if defined, else `&"MP_OVERTIME_CAPS"`).

#### `gf_overtimeCueVO()`
The `"ctf_start"` cue (`game["dialog"]["gf_overtime_cue"]`, registered in `gf.gsc::onPrecacheGameType`) that follows the stock "Overtime" callout, at **1.0s** (a plain `wait()` literal in the function — the only thing setting the gap; too low and the two lines talk over each other). Deliberately **not** a `leaderDialog` call, for two reasons. **Cancellable:** `leaderDialog` pushes onto `self.leaderDialogQueue`, whose drain thread (`playLeaderDialogOnPlayer`) carries only `self endon("disconnect")` — nothing round-scoped — so a queued line is spoken even if the round already ended, which is why a 2-second overtime still announced "capture the flag". Holding it in our own thread lets `gf_ot_done` / `game_ended` kill it first. **Timing:** the queue's spacing is a hardcoded `wait(3.0)` in `playLeaderDialogOnPlayer`, so anything behind "Overtime" lands at exactly 3.0s and no dvar retunes it. Plays the alias directly the same way the stock path builds it (`game["voice"][team]` faction prefix + the dialog key, `"vox_wm_"` under `level.wagerMatch`), gated on `level.allowAnnouncer`.
...
Registered damage hook; returns the (possibly modified) damage. Returns early for non-positive damage. Queues a health HUD update if the victim is alive. Recolours the attacker's stock hitmarker (`.hud_damagefeedback`) before the friendly-fire/state returns (stock draws a marker for FF too): red on a killing blow (`iDamage >= self.health` — health is still pre-damage here), white otherwise, both writes **bare** (⚠ no `fadeOverTime` in the damage frame — a window armed here swallows stock's same-frame `alpha = 1` flash, base alpha 0, and the marker never becomes visible; `fadeOverTime(0)` is equally not a snap — the kill colour is made to stick by the snap thread instead). A killing blow also stamps `eAttacker.gf_redMarkerAt = gettime()` and threads `gf_snapKillMarkerRed( killAt )`. The stamp scopes red priority to the kill's own frame: a non-lethal co-hit with equal `gettime()` skips the white re-stamp (stock's alpha re-flash still runs), so the killing blast's trailing shotgun pellets can't whiten the flash; a white in any later frame re-stamps immediately and clears the stamp (fresh feedback on a new target wins — a dead victim fires no damage events, so only same-frame co-hits can belong to the kill). Returns `iDamage` unchanged for self-damage, missing teams, friendly fire, or non-playing states. If `level.gf_headshotsOnly` is set, returns 0 for non-head/helmet hits. Caps recorded damage at the victim's remaining HP, then accumulates `eAttacker.pers["gf_damage"]` (pushed silently via `gf_setPlayerScoreSilent`), records per-target damage in `gf_dmgOnTarget[victimKey]` for the kill popup, tracks the attacker as a unique assister on the victim, and queues a HUD update.

#### `gf_snapKillMarkerRed( killAt )`
Attacker-side thread (`endon "disconnect"`) started by the killing-blow branch of `gf_onPlayerDamage`; owns making the red actually stick, one frame behind the bare damage-time write. Stock's flash (`_globallogic_player.gsc:968`) re-arms `fadeOverTime(1)` in the same frame as that write, folding a still-in-flight colour lerp into its full 1s fade (the "faded red" rapid-fire kill) — and no window may be armed in the damage frame itself (it would swallow stock's `alpha = 1` instead). One frame later the element is quiet: the thread re-snaps `.color` red on a private `fadeOverTime(0.05)` window, and the frame after restarts the flash (`alpha = 1`, `fadeOverTime(1)`, `alpha = 0` — stock's exact idiom): the restart is mandatory because the snap re-times the in-flight alpha fade too, which would otherwise blink the marker out instead of fading it over its second. `killAt` is the generation token — before each step the thread bails unless `self.gf_redMarkerAt` still equals it, so a later white (which clears the stamp) or a newer kill (which restamps it) retires the thread instead of getting painted over. Step 3, ~1.15s after the kill: bare-writes the element's **resting colour back to white** and clears the stamp — fourteen stock files (the equipment family, in Gunfight's case) flash this element without entering `Callback_PlayerDamage`, so without the reset a persisted red made enemy-equipment hits flash red indefinitely after a kill.
...
Registered as `level.onSpawnPlayer`; selects a spawn point and spawns the player. Sets `self.sessionstate = "playing"`, clears `self.usingObj`, and sets `self.maxhealth`/`self.health = 100`. Resolves `spawnTeam` from `self.pers["team"]`, flipping it via `getOtherTeam` when `game["switchedsides"]` is true. In small mode it tries `gf_getCustomSpawnPoint(spawnTeam)` and, if defined, spawns there with the `"gf"` class and returns; on a curated miss it next tries the map's `mp_wager_spawn` pool via `getSpawnpoint_NearTeam` (telefrag-aware, in-bounds of the wager play area — small mode must never spawn past the baked blockers, which the tdm start points on wager maps are) and returns if that delivers. Otherwise (large mode, or a small-mode map with no wager pool — which also has no blockers) it uses team-specific start spawns: `mp_tdm_spawn_<team>_start`, falling back to `mp_sab_spawn_<team>_start`; if either has points it picks one via `getSpawnpoint_Random`, else it falls back to `getTeamSpawnPoints` + `getSpawnpoint_NearTeam`. Finally spawns the player at the chosen origin/angles with the `"gf"` class. (Team-specific starts are used deliberately so a late-spawning bot isn't placed on the wrong side of a shared pool.)
...
This file owns the entire shared-loadout system: the 53-entry hand-authored loadout pool (every line carries its OWN offhands, camos and optional per-loadout perks — the old build-time offhand rotation is gone), the once-per-match Fisher-Yates shuffle, the deterministic per-round loadout pick, the loadout delivery hook (`level.giveCustomLoadout`), the token→name/icon resolver tables, and the RCON perk-override layer. It `#include`s `maps\mp\gametypes\_gf_hud` (used by `gf_giveCustomLoadout`, which threads `gf_showWeaponHUD`). The pool lines live between `#gf-loadout-editor-begin/end` markers — `tools/loadout_editor` rewrites them (its save-time validator checks perk tokens), hand-editing between them is supported, and `tools/verify_loadouts.ps1` statically validates weapon/perk/camo tokens at commit time (a pre-commit gate — an unknown token is a silent GiveWeapon/SetPerk no-op at runtime).

#### `gf_initLoadouts()`
Builds the pool once per match and stores it in `game["gf_pool"]`, guarded by `game["gf_init"]`. First builds the resolver tables (`gf_buildWeaponDB` → `level.gf_wpnDB`/`_wpnFam`; `gf_buildPerkDB` → `level.gf_perkDB`), then runs the 53 `pool[n] = gf_load( PRI, SEC, EQUIP, LETHAL, TACTICAL, CAMO, CAMO2 [, PERKS] ); n++;` lines (organized by class: AR ×8, SMG ×6, LMG ×4, Sniper ×2, Shotgun ×2, then expanded batches, dual-wield SMGs, and the heavy/special entries), Fisher-Yates shuffles in place, sets `game["gf_pool"]`/`game["gf_init"] = 1`, and drops the resolver tables off `level` (only needed during the build — the pool carries resolved names/icons).

#### `gf_camoPool()` / `gf_dealCamos( pool )` / `gf_shuffledCamoBag( avoidFirst )`
**The camo rotation, and `gf_camoPool` is the edit surface** — it returns the curated array of camo indices the `-1` (Random) loadouts may roll, as an explicit commented list where uncommenting a line adds a camo and commenting one out drops it. Camos are **dealt, not rolled**: `gf_dealCamos( pool )` runs once per match in `gf_initLoadouts` **after** the pool's Fisher-Yates shuffle — so bag order is *play* order — and walks the shuffled pool filling every `-1` slot from two independent shuffled bags (`gf_shuffledCamoBag`), one for primaries and one for secondaries. Every enabled camo is therefore handed out **once before any of them comes round again** (a bag is ~29 deep = 29 loadouts on the primary, far longer than a match), and a refilled bag swaps its first entry away if it matches the value the previous bag closed on, which is the only place a back-to-back repeat could survive. ⚠ The deal **must** come after the shuffle: rolling per slot inside `gf_load` (what this did until 2026-08-20) put the spread on the *authoring* order, which the shuffle then scrambled back into independent-looking noise — the reported "camos repeat too often". A pinned index is never touched, and both bags come from the already class-filtered `gf_camoPool()`, so the two admin switches still govern.

On the way out the list passes through `gf_camoClassFilter` → `gf_camoAllowed`, the two runtime class switches (below). So the source list is the *maximum* set: an index commented out here can never be turned back on at runtime, while an index left in can still be switched off server-side.

#### `gf_camoAllowed( idx )` / `gf_camoClassFilter( p )` / `gf_resolveCamo( idx )`
The two admin camo-class switches, `scr_gf_camo_base` and `scr_gf_camo_modded` (both default ON, empty/unset reads as ON like the slot switches). `gf_camoAllowed` is the single predicate: index `0` (None — the base gun finish, not a camo) is **always** allowed, `15` (Gold) is allowed if **either** switch is on (it belongs to both sets deliberately — the one camo every BO1 player knows), `1`-`14` need base, `16`+ need modded. `gf_camoClassFilter` applies it to the rotation at pool build; `gf_resolveCamo` applies it again at each spawn's give, so a panel flip lands on the **next spawn** instead of the next match (the pool is built once per match). ⚠ A disabled index is remapped **deterministically** — `enabled[ idx % enabled.size ]`, derived from the loadout's own index — and never re-rolled: a roll would hand two players different guns out of the same shared loadout, breaking the sync-by-construction guarantee. Both classes off → every index resolves to `0`, i.e. plain factory finishes. `gf_force_camo` is applied *after* the resolve, so an explicit admin force still wins.

⚠ **This replaced a bare `randomInt( 16 )`, which was a real bug, not a style issue:** it could only ever return 0-15, so every custom camo (17-46) was **unreachable in a live round** no matter how many shipped. If a newly added camo never appears, check this list before suspecting the assets.

⚠ An index in this list **must** have a row in `mp/weaponOptions.csv` **and** a carrier material in `mod.csv`, or it renders **flat white** (a missing camo image is white, not absent) — adding a camo is three files, not one. ⚠ Index `16` is deliberately excluded: it is a diagnostic duplicate of `3` (Red), a stock resident image at an index >15, kept to separate "the index path broke" from "the image never arrived". ⚠ Keep the loadout editor's upper bound (`tools/loadout_editor/server.js`) and the RCON panel's Force Camo list in lockstep with the last `gf_camo_` row. Architecture → `docs/notes/custom-camos-bocl-architecture.md`.

#### `gf_load( pri, sec, equip, lethal, tactical, camo, camoSec, perks )`
Assembles ONE pool entry. Weapon tokens resolve through `gf_wdb()` (display name + HUD shader, attachment variants resolve for free via the family table). Camo: a pinned index (`0-15` stock, `17-46` this mod's own), `-1` defers to `gf_dealCamos` (dealt from a shuffled bag after the pool shuffle, so consecutive loadouts never repeat a camo); primary and secondary draw from separate bags; the Minigun/M202 specials and the Finger Gun (`defaultweapon`) are forced to stock camo (they reject/ignore a real one). `equipNone` is carried for the HUD (equipment is the only slot that may be `"none"`). The optional 8th field is a comma-separated specialty list parsed once at build into `perkAdds`/`perkRems` (a leading `-` removes a base perk). The overview's three perk tiles are the SAME fixed trio for every loadout — Flak Jacket / Hardened / Marathon Pro, one representative per tier color (the kit is shared; `specialty_armorvest`/"Body Armor" is a mode rule, not a tile, and its true art doesn't load in MP).

#### `gf_pickLoadout()`
Selects the active loadout for the current round into `level.gf_currentLoad`. Returns early if `game["gf_pool"]` is undefined **or empty** (an emptied pool would otherwise modulo-by-zero every round). Deterministic by construction: `idx = int( game["roundsplayed"] / level.gf_cfg_roundsPerLoadout ) % game["gf_pool"].size` — derived purely from the persisted round counter, so calling it multiple times in a round always yields the same loadout, and the loadout rotates every `level.gf_cfg_roundsPerLoadout` rounds. Dev override: `gf_force_loadout` pins an index (strip-marked).

#### `gf_giveCustomLoadout()`
The loadout-delivery hook registered as `level.giveCustomLoadout`; called by `_class::giveLoadout` with `self` = the spawning player. Returns early if `level.gf_currentLoad` is undefined or if the player's `self.pers["team"]` is not `"allies"`/`"axis"`. Reads `load = level.gf_currentLoad`, then `self maps\mp\gametypes\_wager::setupBlankRandomPlayer( true, true )` to clear the player and assign a random body. Computes packed camo options for primary and secondary via `CalcWeaponOptions( gf_resolveCamo( load["camo"] ), 0, 0, 0 )` and the same for `load["camoSecondary"]` (stock lens/reticle) — the `gf_resolveCamo` pass is where the two camo-class switches land on a live match. Wraps the weapon grants in `DisableWeaponCycling()` / `EnableWeaponCycling()`. Gives primary (`GiveWeapon( load["primary"], 0, camoOpts )`), secondary (with `secCamoOpts` — only shows on real-base secondaries like crossbow, no-op on neutral pistols/launchers), and `knife_mp`, then `switchToWeapon( load["primary"] )` (no `giveMaxAmmo` — relies on default reserve ammo). Gives the lethal with `setWeaponAmmoClip` of `lethalCount` = 1, except `hatchet_mp` which gets 2, then `SwitchToOffhand( load["lethal"] )`. Gives the tactical clamped to 1. Each of the three grenade/equipment slots is additionally gated on its **admin slot switch** — `gf_slotOn( "scr_gf_lethals" / "scr_gf_tacticals" / "scr_gf_equipment" )`, `0` = that whole slot is off server-wide; the dvars are read here at spawn rather than cached on `level` so a panel toggle lands on the very next spawn, and `_gf_hud` reads the same three to hide the matching overview slots (it cannot call in here — the include graph is one-way, loadouts → hud). Equipment (`load["equip"]` + `SetActionSlot( 1, "weapon", ... )`) is given only when the player is NOT a bot (`self.pers["isBot"]`) **and** the slot isn't empty — `gf_slotEmpty( token )` is true for the `"none"` token the loadout editor writes (equipment is the only slot that may be empty; `GiveWeapon` on an unknown token hands out the finger gun rather than nothing, so the grant must be skipped outright). Then sets the base perk set (9): `specialty_fallheight` (Lightweight Pro / no fall damage — ⚠ the speed half `specialty_movefaster` is deliberately NOT granted since 2026-07-16; the +7% made 42s rounds twitchy, and an admin opts it back in via `gf_perk_on`), `specialty_longersprint` (Marathon), `specialty_unlimitedsprint` (Marathon Pro / the sprint meter never empties — the engine's own perk bit, consumed by engine movement code; independent of the `scr_gf_sprint_unlimited` client-dvar path, which stays until this is proven live and can then be retired), `specialty_armorvest`, `specialty_flakjacket` (Flak Jacket), `specialty_shades` + `specialty_stunprotection` (Tactical Mask Pro's flash/stun halves), `specialty_loudenemies` (Ninja Pro's listener-side half — everyone hears everyone louder, symmetric), and `specialty_fastmeleerecovery` (Steady Aim Pro's melee half). The per-loadout `perkAdds`/`perkRems` layer applies after the base set (the sniper/heavy package rides here, incl. `specialty_bulletflinch` — never in the base set, it double-reduces flinch under `scr_gf_flinch`). ⚠ `specialty_armorvest` is **not** Flak Jacket, and is **not any Black Ops perk** — it is an engine leftover token (no create-a-class row, no icon) with live damage code, which we label **"Body Armor"** after its effect. It applies a flat **−20% on every non-headshot bullet hit** (`_class::cac_modified_damage` → `damage * perk_armorVest * .01`, dvar defaults to 80), applied on the live stock damage path. It is granted **on purpose** (symmetric, and a softer bullet TTK suits a 42s round), accepting that headshots bypass it entirely and therefore skew both damage-scoring and the most-remaining-HP round decision. `specialty_flakjacket` is the real Flak Jacket (explosive reduction); its Pro would be `specialty_fireproof`, which is not granted. Applies the RCON override layer last via `gf_applyPerkList( getDvar("gf_perk_on"), true )` and `gf_applyPerkList( getDvar("gf_perk_off"), false )` so admin toggles win over the base set. Finally `self thread gf_showWeaponHUD( load )` to slide in the loadout HUD.
...
#### `gf_wdb( token )` / `gf_buildWeaponDB()` / `gf_buildPerkDB()`
The resolver layer that lets a pool line carry only weapon TOKENS. `gf_buildWeaponDB` registers each base weapon once (`gf_reg`: exact token → name + shader; `gf_regFamily`: base name → name + shader, so any attachment variant like `famas_gl_mp` resolves to "FAMAS" + the FAMAS icon for free). `gf_wdb(token)` looks up exact-first then family, returning the `["w"]/["n"]/["s"]` triple `gf_load` copies onto the entry; an unknown token still returns a usable triple (token as name, white shader) so a typo degrades visibly rather than crashing — existence is enforced at commit time by `tools/verify_loadouts.ps1` instead. `gf_buildPerkDB` (`gf_pReg` rows) maps specialty tokens → display name + icon-parent (a Pro has no art of its own and borrows its parent's). All three tables are build-time only and are dropped off `level` once the pool is cached.

#### `gf_slotOn( dvar )` / `gf_slotEmpty( token )`
`gf_slotOn` reads a per-slot admin switch (`scr_gf_lethals`/`scr_gf_tacticals`/`scr_gf_equipment`) live at each spawn's give (empty = on). `gf_slotEmpty` is true for the `"none"` token — equipment is the only slot allowed to be empty, and the give must be skipped outright (`GiveWeapon` on an unknown token hands out the finger gun rather than nothing).
...
Per-player thread that shows the menu-rendered create-a-class loadout overview, then hides it. No-ops if `load` is undefined. Fires/`endon`s `"gf_kill_loadout_hud"` (singleton), `endon`s `"disconnect"`, `endon`s `level "game_ended"`. Pushes 8 icon materials (`ui_gf_lo_icon0..7`: primary, secondary, lethal, tactical, equipment, then the three fixed perks Flak Jacket/Marathon/Lightweight via `gf_getPerkShader`) and 8 names (`ui_gf_lo_name0..7`), plus the column anchor `ui_gf_lo_cx = -104` / `ui_gf_lo_cy = -6`. Also pushes the three equipment-row slot gates `ui_gf_lo_show2` (lethal) / `_show3` (tactical) / `_show4` (equipment) — `0` hides that column's icon + name while the row's bracket still spans all three, so the slot simply reads empty and the block never reflows. A gate is `0` when the loadout carries `load["equipNone"]` (equipment token `"none"`) or when that whole slot is switched off server-wide (`scr_gf_lethals` / `scr_gf_tacticals` / `scr_gf_equipment`, read here with `getDvar` — the same dvars `_gf_loadouts::gf_slotOn` skips the give on). The intro is a snap (`ui_gf_lo_off 0`, `ui_gf_lo_alpha 1`, `ui_gf_lo_show 1`), pushed as **two** batched groups: every gate/anchor value first, then a lone `ui_gf_lo_show 1` command, so the reveal can never race a stale gate (the gates took the group past the ≤8-pair batching rule). Dismissal is anchored to **go-live and then to the player's own presence**, never to a fixed timer from the spawn: it waits out the countdown (`level waittill "prematch_over"`, gated on `level.inPrematchPeriod` so a mid-round late spawn doesn't park forever), then `gf_waitForPlayerPresent( 5000 )`, then `wait 1`, and slides+fades out via `gf_slideLoadout( 0, 70, 1, 0, 0.5 )` and sets `ui_gf_lo_show 0`. The old flat `wait 8` ran on a clock the player may not be on yet — this spawn is the frozen prematch spawn, and round 1's countdown is 20s, so it retired the overview ~12s *before* go-live for everyone, and a client still finishing its first-connect load could miss it entirely. Rounds 2+ (7s countdown) land at 7+1 = 8s, i.e. exactly the old behavior, so the change is surgical to round 1. Measure the residual client-readiness gap with `gf_debug_loadgap` (`GF_LOADGAP`).

#### `gf_waitForPlayerPresent( timeoutMs )`

Blocks until the player gives evidence their client is actually up and they are present, or `timeoutMs` elapses. **Called, not threaded**, so the caller's `endon`s govern it. ⚠ **The server cannot see "the client is rendering"** — `statusicon`/`"begin"` (`_globallogic_player.gsc:19-21`) is the deepest readiness signal GSC gets and fires at cgame init, before the first rendered frame, Plutonium's in-place engine rebuild and the Demonware resync (stock doesn't try — T5's `waitForPlayers()` is an empty stub). So it watches a **proxy**: the usercmd stream, via view-angle divergence (>2°) off a baseline, an origin delta (>1 unit), or `Attack`/`Jump`/`MeleeButtonPressed`. ⚠ **Call only after go-live** — during the frozen prematch a still player is indistinguishable from a client sending nothing, so it would just burn the timeout. ⚠ **It fails safe, which is what makes it usable for a HUD tail and NOT for the match-start load gate**: "no input" never means "not ready", only "no evidence", so an ambiguous negative costs one player a slightly longer panel here, whereas as a gate release condition it would make the whole lobby pay the ceiling because one person didn't move — and it cannot see the pre-`"begin"` clients that are the gate's actual subject. ⚠ Builtins only, all MP-verified; **not** `GetNormalizedMovement` (SP-only, fails the server with `unknown function`). The angle compare is a deliberate naive subtraction — for change detection the 0/360 wrap can only inflate a delta into a detection, never hide one, so no yaw-wrap helper is needed.

#### `gf_slideLoadout( offFrom, offTo, alphaFrom, alphaTo, dur )`
Per-player linear slide+fade of the whole overview over `dur` seconds in 0.05s frames (`steps = int(dur/0.05)`, min 1). The menu adds `ui_gf_lo_off` to every item's X and multiplies `ui_gf_lo_alpha` into every item's alpha, so this drives the block as one. Raises `ui_gf_lo_show` on the first frame (so the block is never seen parked), keeps `ui_gf_lo_off` fractional (no `int()` rounding, to avoid uneven 20Hz steps), then snaps to the final `offTo`/`alphaTo`. Drives the loadout **outro only** (the intro snaps).

⚠ **This is a reliable-command STREAM** — the densest one the mod emits. Each step's `off`+`alpha` pair is pushed as **one batched `setClientDvars`**, so the 0.5s outro costs **13** reliable commands per human per round (it was 26 unbatched). Never expand the pair back into two pushes. ⚠ **But note the window:** it fires ~8s into the round, mid-gameplay — NOT in the `map_restart(false)` lobby-START stall where the overflow actually bites, so it is a purity cost, not a live-problem cost. The zero-cost end-state is to let the **menu layer** own the animation (`milliseconds()` in an `exp`), blocked on the client-local-clock question (see CLAUDE.md).
...
Holds the per-map curated Gunfight spawn sets and overtime flag points (the `gf_locationsTable()` data table, one block per map), plus the helpers that load, validate, and dispense them at runtime. It defines no `#include` directives and calls only engine builtins (`getDvar`, `logPrint`, `isDefined`). The curated data is consumed by the spawn pipeline and overtime system in `_gf_rounds.gsc`/`gf.gsc`, which read the `level.gf_customSpawns` / `level.gf_customOvertimeLocation` state this file populates.

#### `gf_initCustomLocations()`
Entry point that loads and prepares all custom-location state for the current map. On the FIRST round of a match it builds `gf_locationsTable()` **once**, hands the table to both getters (`gf_getCustomSpawnLocations(t)` → `level.gf_customSpawns`, `gf_getCustomOvertimeLocation(t)` → `level.gf_customOvertimeLocation`), runs the two validators, and caches the results in `game[]` (`gf_customLocCached`/`gf_customSpawnsCache`/`gf_customOvertimeCache` — the map-constant data used to be rebuilt+revalidated on every `map_restart`); later rounds just restore the cached references. Always resets the per-round runtime state (`gf_customSpawnRound = -1`, `gf_customSpawnCursor = []`).

#### `gf_locationsTable()` / `gf_locMapEntry()`
THE curated-data table: one block per map (25 maps today), each `t["mp_<name>"] = e` entry built from `gf_locMapEntry()` (an `e["sets"]` list — every map ships one set of 5 allies + 5 axis `gf_sp` points today; `gf_getCustomSpawnPoint` round-robins sets by `roundsplayed % sets.size` if more are ever added) plus `e["ot"]`, the overtime flag point. Data lines are the spawn recorder's own paste-ready output (`gf_debug_spawns 1`, ActionSlot3 — edit coordinates only through it). An UNLISTED map is the supported opt-out: no entry → small mode degrades to the stock pools and the OT flag falls back to the native Dom-B point. The table's shape contract (exactly 5+5+1 per map, header comment matches the key, yaw normalized) is enforced at commit time by `tools/verify_locations.ps1` (pre-commit gate); the runtime validator below is deliberately looser.

#### `gf_getCustomSpawnLocations( t )` / `gf_getCustomOvertimeLocation( t )`
Thin lookups into the pre-built table for the current `mapname`: the first returns `result["sets"]` (empty for an unlisted map), the second the `gf_ot` point or `undefined`. Both take the table as a parameter — `gf_initCustomLocations` builds it once and shares it (each used to build its own copy). The old flat `allies`/`axis` result arrays and their normalize/promote pipeline (`gf_normalizeCustomSpawnLocations`, `gf_getCustomSpawnCount`) are DELETED — they were empty by construction and every branch that serviced them was unreachable.
...
Dispenses the next curated spawn point for `team` ("allies"/"axis"), cycling through the active set's list round-robin. Returns `undefined` if no custom spawn data exists (`level.gf_customSpawns` / `["sets"]` undefined, or zero sets). It derives `roundKey` from `game["roundsplayed"]` (defaulting to 0); when the round changes (`level.gf_customSpawnRound != roundKey`) it stores the new round and resets both per-team cursors (`level.gf_customSpawnCursor["allies"/"axis"] = 0`). It selects the set with `setIndex = roundKey % sets.size` (so sets rotate by round when more than one exists), returns `undefined` if that set's team list is empty, otherwise scans forward from the cursor for the first point that would NOT telefrag (`positionWouldTelefrag`) and post-increments the team cursor. If EVERY curated point is occupied (possible with 6 bodies a side on 5 points) it returns `undefined`, sending the caller down its in-bounds fallback chain — the `mp_wager_spawn` pool first, stock `mp_tdm_spawn_<team>_start` only on maps with no wager pool — it never spawns onto an occupied point (that kills the occupant).

#### `gf_validateCustomLocations()`
Runtime sanity net (deliberately looser than the commit-time `verify_locations.ps1` contract): iterates the loaded sets counting team points with `gf_getCustomSpawnSetCount()`, keeps only sets with at least one point per team (a dropped set gets a `logPrint` warning), rewrites `["sets"]` to the survivors, and logs a load summary when any remain. No surviving sets = an unlisted map — the supported opt-out — and stock spawns take over silently.
