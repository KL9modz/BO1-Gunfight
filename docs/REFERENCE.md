...
Per-player thread that shows the menu-rendered create-a-class loadout overview, then hides it. No-ops if `load` is undefined. Fires/`endon`s `"gf_kill_loadout_hud"` (singleton), `endon`s `"disconnect"`, `endon`s `level "game_ended"`. Pushes 8 icon materials (`ui_gf_lo_icon0..7`: primary, secondary, lethal, tactical, equipment, then the three fixed perks Flak Jacket/Marathon/Lightweight via `gf_getPerkShader`) and 8 names (`ui_gf_lo_name0..7`), plus the column anchor `ui_gf_lo_cx = -104` / `ui_gf_lo_cy = -6`. Also pushes `ui_gf_lo_show4` — `0` when the loadout carries `load["equipNone"]` (equipment token `"none"`), which hides the overview's equipment icon + name; the equipment row's bracket still spans all three columns, so the slot simply reads empty. The intro is a snap (`ui_gf_lo_off 0`, `ui_gf_lo_alpha 1`, `ui_gf_lo_show 1`). After `wait 8`, it slides+fades out via `gf_slideLoadout( 0, 70, 1, 0, 0.5 )` and sets `ui_gf_lo_show 0`.
