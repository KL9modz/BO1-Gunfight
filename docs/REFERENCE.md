...
Because `pauseTimer` freezes `getTimePassed`, the stock grenade/launcher dud window would mis-fire; `gf_startRoundClock` disables it by setting `level.grenadeLauncherDudTime`/`thrownGrenadeDudTime` to `-1`. The per-round prematch is MOD-OWNED (`gf_prematchCountdown`): `level.prematchPeriod` is pinned to 0 and a `gettime()` clock draws the countdown number, beep and vision blend, with a partial movement lock in place of the stock full freeze. Key dvars: `scr_gf_timelimit`, `scr_gf_timelimit_large`, `scr_gf_match_prematch_seconds`, `scr_gf_prematch_seconds`; key vars: `level.gf_roundRemaining`, `level.timeLimitOverride`.
...
The **mod-owned prematch**, replacing the engine's `matchStartTimer`. Threaded from the tail of `onStartGameType` (after the strip-marked hold, so the dev lobby pass threads none). Waits out the engine's t≈0 `prematch_over` fire, then in one yield-free block stamps `level.gf_enginePrematchFired`, re-asserts `level.inPrematchPeriod`/`inGracePeriod`, sets `timeLimitOverride` + `pauseTimer()` + `setGameEndTime(0)` (the engine would otherwise run its 42s time-limit machinery through the countdown), snapshots and zeroes `jump_height`, and forces `visionSetNaked("mpIntro", 0)`. Builds two `createServerFontString` elements (label + number, no per-client reliable commands), then loops at 0.05s recomputing `secs` from `gettime()` — so a frame hitch skips a display value instead of dilating the clock — beeping via `gf_countdownTickAlias()` and blending to `gf_visionSetForKey(gf_roundVisionKey())` over 3s at T-2s. Ends by calling `gf_prematchGo`. ⚠ Never `endon("prematch_over")` here: `gf_prematchGo` notifies it, which would kill the caller.

#### `gf_prematchIsActive()` / `gf_prematchGo( myGen )` / `gf_prematchLockWatchdog( myGen, endTime )`
`gf_prematchIsActive` reads `level.gf_prematchActive` — the lock keys off **this**, not `inPrematchPeriod`, which lies for ~1 frame at t≈0. `gf_prematchGo` is the mod's copy of stock `prematchPeriod()`'s tail: latches the flag false (idempotent), restores `jump_height`, releases every player's lock, unfreezes, re-pushes the objective hint, re-fires the intro VO, and **last** notifies `prematch_over`. `gf_prematchLockWatchdog` forces GO 10s past schedule if the countdown thread dies, so a dead thread can never leave players locked.

#### `gf_prematchLockOnSpawn()` / `gf_applyPrematchLock()` / `gf_releasePrematchLock()` / `gf_restoreJumpHeight()`
The **partial lock**, threaded per spawn from `gf_playerSpawnedCB` (bots included). Waits stock's `spawned_player` entity notify so the loadout is complete, then swaps the stock full freeze for: `setMoveSpeedScale(0)` (guarded — never snapshots a non-positive scale, which would freeze the player for the match), `DisableOffhandWeapons()`, ammo zeroed with clip/stock snapshots, and a humans-only `ammoCounterHide 1` push so the zeroed clips can't render "NO AMMO". Look/ADS/stance/switch/melee stay live by design. Release restores ammo only on weapons still carried, and `gf_restoreJumpHeight` undoes the level-wide jump lock on both the GO and bail paths. ⚠ The jump lock is `jump_height 0` level-wide, **not** `AllowJump` — that builtin has zero `raw/maps/mp` hits and would fail the whole server at compile time.
