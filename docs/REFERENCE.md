...
| `scr_gf_teamspawnmode` | `auto` | `auto` \| `large` \| `small` | both | Spatial mode selector. `auto` goes large once the larger team exceeds the health-panel skull cap (`gf_hudSkullCap()` = 4), i.e. any team of `5+`. An invalid value is rewritten to `auto`. `large`/`small` pin the mode. |
| ~~`scr_gf_largemode_minplayers`~~ | *(retired)* | — | — | **RETIRED — no longer read.** The small/large split is hard-wired to the skull cap in `gf_autoLargeFromCounts()` (`<=4v4` small, any team of `5+` large); a tunable threshold could only decouple it from the fixed menu `cnt > 4` gate. Any stale cfg value is inert. Pin the mode with `scr_gf_teamspawnmode`. |
