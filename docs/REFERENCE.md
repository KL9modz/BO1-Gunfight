...
| `scr_gf_headshot_scale` | `1.0` | `0`–`3` | both | Multiplier on **final** headshot damage. The engine applies the weapon file's baked hit-location multiplier before GSC sees the damage (`MOD_HEAD_SHOT` is set at `_globallogic_player.gsc:731-738`, before the `level.onPlayerDamage` hook at `:741`, whose return value `:742-743` takes as the applied damage) — so this rescales that result globally (`1` = stock, `0.5` = half, `0` = headshots void; a positive scale never rounds a hit below 1 damage); it cannot set a weapon's multiplier to an absolute value. Applied at the top of `gf_onPlayerDamage` so the scaled value flows through the damage score, the most-HP time-out decision, and the red kill-marker test (FF headshots scale too — the engine applies FF damage even though it is never scored). Read live per hit (`gf_headshotScale`), so an RCON `set` lands on the next shot; registered per round from `gf.gsc::gf_roundApplyTuning` for the panel sweep. Interacts with Body Armor: everyone takes −20% on non-headshot bullets, so headshots are already worth proportionally more than the weapon files say — lowering this narrows that deliberate gap. |
