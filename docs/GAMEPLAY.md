...
Black Ops Gunfight is a standalone round-based gametype (`gf`) for Call of Duty: Black Ops 1 on the Plutonium T5 client. This document covers everything that defines how the mode plays. For setup see [Setup](SETUP.md); for the full dvar/variable list see [Reference](REFERENCE.md); for developer internals see [Dev](DEV.md).
...
The player- and admin-relevant gameplay dvars. Set them in the server config or via the console. For the complete list (including HUD, perk-override, and visual dvars) see [Reference](REFERENCE.md).
