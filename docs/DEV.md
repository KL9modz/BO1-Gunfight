...
- **Match → match teams are controllable (carry, shuffle + staging).** Between matches the map change
  wipes `pers[]` and ranked connect re-runs stock autoassign — with balanced counts that is
  `pickTeamFromScores` → `randomInt(2)`, a per-player coin flip. `gf_team_nextmatch`
  (`stock`|`keep`|`shuffle`, panel ADVANCED → TEAMS) puts it under policy: **keep** snapshots teams on
  the match-ending round (`_gf_rounds::gf_writeNextMatchPlan`, from `gf_onRoundEndGame` — stock's
  match-end-only callback, so it covers the bridge END ROUND path that bypasses `gf_endRound`) into `gf_teamplan` +
  the `gf_teamcarry` marker; **shuffle** writes the same-shape plan but deals the seated humans onto a
  random balanced split every match (`gf_writeShuffledTeamPlan`, Fisher-Yates, off-by-1-even, odd
  extra on a random side, spectators left spectating); the panel's sidebar **NEXT MATCH** block stages an explicit layout into
  **`gf_teamstage`** (one raw `set`, no bridge verb), which is one-shot and wins over both at the
  next match start — map rotate, lobby release, and bridge `matchrestart` all honor it.
  `_gf_rounds::gf_consumeTeamPlan` consumes (and clears) all three dvars on the consuming match's
  first pass; seating rides the existing connect-time pre-spawn plan path (`gf_autoassignPlanned`),
  so there is no sequenced suicide/respawn and no prematch flicker. Bots need no plan — the
  reconciler re-pads from `gf_fill_n`.
...
- `bot_set_difficulty()` (`easy` / `normal` / `hard` / `fu`) is the dvar set behind the bridge's `botdiff_*` commands. It rewrites the whole `sv_bot*` preset from whatever `bot_difficulty` holds, and `diffBots` re-runs it every 1.5s — so `bot_difficulty` always reflects the live difficulty (the panel reads it on connect to highlight the right button; the buttons carry the game's Combat Training names Recruit/Regular/Hardened/Veteran). ⚠ `bot_difficulty` is a **REAL ENGINE dvar** (BO1 Combat Training), registered at process start — default `normal`, **closed** enum domain easy/normal/hard/fu (live rcon read 2026-07-17; a fifth name is domain-refused). It is therefore never empty, and the seed-if-empty `gf.gsc` used to carry was **dead code that never fired**: the VPS's old "fu" was a live panel `botdiff_fu` click surviving in-process, silently reverted to `normal` by the next server restart. The GF default is **`hard`** (Hardened, down from `fu`/Veteran as of 0.8.3), owned by `dedicated.cfg` (a genuine deviation from an engine default — cfg's job); a live `botdiff_*` click still wins until the next restart.
- **The `gf_sv_<dvar>` mirrors are the per-dvar override layer on top of that preset — the CUSTOM difficulty's definition, applied only while the selector `gf_bot_difficulty` reads `custom`** (its base is the fu table; the four stock `botdiff_*` verbs write the `"stock"` sentinel so Recruit→Veteran run pure designed values; `botdiff_custom` selects it; tuning any `sv_bot*` via `svset_` auto-switches to it; GSC seeds `custom`, but **`dedicated.cfg` ships `stock` as of 0.8.3** so the Hardened default runs pure). ⚠ **`botdiff_custom` hardcodes `bot_set_difficulty("fu")`**, so CUSTOM does not mean "current preset + overrides" — it *sets* the difficulty to Veteran and layers the overrides on that. Deliberate (CUSTOM = Veteran plus our adjustments), but it means one click leaves the Hardened default until a `botdiff_*` click or a restart, and the `svset_` auto-switch is the one path that applies the mirrors on a non-fu preset. Right after each preset pass (same frame, every 1.5s tick and every round), `diffBots` calls `_gf_bridge::gf_bridgeApplyServerDvars()`, which copies every **non-empty** `gf_sv_*` mirror onto its real dvar — so an override wins continuously, a blank mirror defers to the preset, and a raw `set sv_bot*` (rcon or cfg — **not** cheat-refused on the dedicated console, proven live 2026-07-20) is clobbered within 1.5s. Allowlist = mirror list = `gf_bridgeServerDvarList()`: FOV, reaction/fire min+max, strafe chance, sprint/melee dist, yaw hip+ADS (ADS dominates — `sv_botMin/MaxAdsTime` is pinned 3000/5000 at every difficulty), pitch envelope up/down, grenade gate (+ `timescale`). Clear one with `set gf_sv_<dvar> ""` (the panel's ↺ on an svset row does exactly this — never a "default value" push, which would pin an override). Shipped: `gf_sv_botYawSpeed 12` + `gf_sv_botYawSpeedAds 10` over the fu baseline (cfg-owned).
...
- **Watched by `watchdog.ps1` check 1d.** The watchdog (3-min cadence) alerts on a **restart** — the event that orphans every open session, because a session is a CHILD PROCESS of this server with no server-side snapshot to reattach to, so the app can only say *Environment deleted* — and also on **two servers** (the ambiguity below) and on a **non-`svchost` parent**. An absent server is Stop/Start-restarted, which bounds an outage at ~3 min instead of this task's own 5-min poll. ⚠ **Not** wrapped in `run_service.ps1` like every other service: that wrapper reparents its target under `powershell.exe`, which breaks Remote Control outright — so the check exists *instead of* a flight-recorder log, and `storage\t5\logs\services\` deliberately has no `GF-ClaudeRC.log`. ⚠ The check treats missing evidence (query failed, command lines unreadable) as **no judgement**, never as "gone": remediation ends live sessions, so a false positive destroys what it protects.
- ⚠ **If `gf-vps` dies while you are away, the watchdog restarts it first** (previous bullet); **SSH is the fallback for when the watchdog itself is down** — that is why port 22 is open to any IP (`SSH-Any-In (travel)`) and why **Blink Shell → `ssh gf-vps` → `claude`** stays the break-glass path. ⚠ A travel device needs the `gf-vps` `~/.ssh/config` block first (snippet in [[vps-three-ips-dont-confuse-vnc-with-vm]]; the `HostName` value is the *Target box* table in `docs/VPS_DEPLOY.md`) — set it up **before** you travel, not from the airport.
