...
This guide covers the contributor side: the repo layout, building `mod.ff`, the branch/release model, the deploy pipeline, and the dev-only tooling (RCON, bots, debug). For per-function and per-dvar detail, see [Reference](REFERENCE.md). For gameplay behavior see [Gameplay](GAMEPLAY.md), and for installing/running a server see [Setup](SETUP.md) and the ops runbooks [../VPS_DEPLOY.md](../VPS_DEPLOY.md) and [../VPS_HARDENING.md](../VPS_HARDENING.md).
