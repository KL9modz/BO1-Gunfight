...
// One table entry = one curated map: entry["sets"] is the per-round spawn-set list
// (round-robin by roundsplayed % sets.size in gf_getCustomSpawnPoint; every map ships
// one set today) and entry["ot"] is the overtime flag point. An UNLISTED map is the
// supported opt-out: no curated data, small mode degrades to the stock pools and the
// OT flag falls back to the native Dom-B point.
gf_locMapEntry()
{
    e = [];
    e["sets"] = [];
...
// ─── THE CURATED-DATA TABLE — one block per map, spawn sets + OT flag together ─────────
// Paste-ready blocks come from the in-game spawn recorder (gf_debug_spawns 1, ActionSlot3
// prints exactly this format — _gf_debug::gf_recPrint). Data lines are the recorder's own
// output; edit coordinates only through it.
gf_locationsTable()
{
    t = [];
    ...
    set["axis"] = x;
    e["sets"][ e["sets"].size ] = set;
    e["ot"] = gf_ot( (4480, 1315, 288), 1 );
    t["mp_villa"] = e;
