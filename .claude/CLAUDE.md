...
3. **Bots pad both sides to `T = max(bigger human side, gf_fill_n)`** — humans define the size, bots
   ...
   fewer humans; default difficulty is **Hardened** (`bot_difficulty hard`, set by `dedicated.cfg` — it
   is an **engine-registered** dvar, so a seed can't own it; see the dvar table). **`gf_fill_n 0` = no
