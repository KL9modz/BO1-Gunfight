...
| `gf_teamplan` | "" | Read-only plumbing for the lobby->match transfer: a `"<guid>:<a\|x\|s>,..."` snapshot of arranged HUMAN teams, written by `gf_writeTeamPlan()` just before the lobby's `map_restart(false)` and consumed once by `gf_applyTeamPlan()` after. A dvar because `game[]`/`pers[]`/`level[]` are all wiped by that restart. Humans only (bots -> `gf_botplan`) |
| `gf_botplan` | "" | Read-only plumbing — the BOT companion to `gf_teamplan` (added 2026-07-10). A `"<alliesN>,<axisN>"` snapshot of the **count** of manually-arranged bots per team, written by `gf_writeBotPlan()` just before the lobby's `map_restart(false)` and consumed once by `gf_applyBotPlan()` after (`_gf_rounds.gsc`, self-contained — reuses `gf_planApplyMove`, no `_bot.gsc` dep, so it compiles in the bot-stripped public build as a no-op). **Count-based, not guid-based** — bots all have guid `"0"` and are fungible, so only the per-side headcount is preserved, not identity. Bots survive `map_restart(false)` as connected clients (only their team placement is wiped), so the apply just re-seats the surviving bots: first A to allies, next X to axis, extras to spectator. Polls fast during the post-restart prematch to catch each bot the instant autoassign resolves its team but BEFORE it spawns (quiet pers reassign, no visible suicide); an already-spawned bot gets the same harmless warmup switch + respawn recovery the human transfer uses. **This is what makes manually-added lobby bots carry into the match with dynamic fill OFF** (`gf_fill_n 0`). **Inert when `gf_fill_n > 0`** — the reconciler owns bot placement and re-pads symmetric N there, so `gf_applyBotPlan` stands down to avoid fighting it |
