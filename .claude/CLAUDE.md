...
| `scr_gf_lobby_hold` | 0 | **Admin lobby hold — the "pregame lobby" for a dedicated server** (`gf_waitForLoadingClients` third release condition, `_gf_rounds.gsc`; `lobbystart`/`gf_bridgeLobbyStart` in `_gf_bridge.gsc`; added 2026-07-05). `0` = **Auto** (default): the pre-prematch hold releases on load + `scr_gf_min_players` as before. `1` = **Manual**: the match's FIRST round HOLDS at "Waiting for teams…" (before the prematch countdown) and does **not** auto-release on load/headcount — it waits for the admin's **START MATCH** click (RCON panel → Match Control rail → bridge `lobbystart`, processed by the 20Hz bridge poll → sets `level.gf_lobbyStart`, which the gate re-checks every 0.25s). Lets an admin arrange teams (right-click → move; applied **live** because `inPrematchPeriod` is already true during the hold) before starting. **10-min `GF_LOBBY_MAX_HOLD` backstop** so a forgotten hold can't wedge the box. In **either** mode, START MATCH is an **instant override** (start now, even below min-players). The gate mirrors its live state into a 7th `gf_state` field (`level.gf_inLobbyHold`) so the panel reveals START MATCH only while a hold is up. Match-start only; clamped 0-1. Needs the dev RCON bridge (Manual is inert on a public/bridge-less build — default Auto is unaffected; the 10-min backstop still recovers if someone force-sets `1`) |
