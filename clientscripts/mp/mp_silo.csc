...
//
// GF OVERRIDE of the stock clientscripts/mp/mp_silo.csc (extracted verbatim from mp_silo.ff).
// ONE change: main() no longer threads rocket_manager(), so the ambient background missile
// launches (exploders 100-105, first at 20-40s after level load - i.e. round starts) never fire.
// Everything else, including every function, is kept byte-for-byte: an override must preserve
// the script's entire public surface (see the stock-script-override rule in CLAUDE.md).
// The two ambient open hatches (rocket_init, exploders 106/107) still run - they double as a
// liveness check that THIS copy compiled and loaded (hatches open = our script is live).
...
main()
{
	...
	// GF: ambient missile launches disabled - rocket_manager() deliberately not threaded.
