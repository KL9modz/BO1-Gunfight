...
const SRV_SECTIONS = [
  { title: 'GENERAL', eff: 'live', per: 'dvar', vars: [
    ...
    // def '0' to match the shipped dedicated.cfg — see the sv_disableClientConsole note below for why
    // a wrong default on a security-shaped dvar is dangerous, not just cosmetic.
    { n:'g_allowvote',              lbl:'Allow Voting',           type:'tog',    def:'0',   tip:'g_allowvote\n1 = players get a CALL VOTE button in the in-game (ESC) menu: Restart Map, Next Map, Change Gametype+Map, Kick Player. SHIPPED DEFAULT: 0.\nStock engine feature — the mod has zero vote code. Takes effect live (~5s): stock _serversettings re-polls it and republishes the ui_allowvote serverinfo dvar the client menu reads. No restart needed.\n⚠ The button is hidden during WAGER gametypes regardless of this (the menu gates on !xblive_wagermatch), and spectators cannot call a vote.\n⚠ A passed "Restart Map" vote fires a RAW engine map_restart, not our endGame path — the same shape as the double-prematch/thread-stacking hazard the bridge restarts avoid. Test that before opening voting to a public lobby.' },
    ...
    { n:'sv_voicequality',          lbl:'Voice Quality (1–9)',    type:'num',    def:'8',   tip:'sv_voicequality\n1 = lowest (least bandwidth), 9 = highest quality. Engine default 3. GF default: 8 -- voice traffic shares the sv_maxRate budget, and that budget is what keeps the between-rounds snapshot gap from flashing "Connection Interrupted".' },
  ...
  { title: 'ENGINE GAMEPLAY', eff: 'live', per: 'dvar', vars: [
    ...
    // ⚠ PLAYER PROFILE SWITCH -- the highest-blast-radius row on this panel, because 0 means the
    // server reads and writes players' REAL Black Ops profiles instead of a private per-mod one.
    // eff:'restart' is deliberate: whether the engine latches it is UNVERIFIED, and clients pick
    // their stat set at connect, so cfg + restart is the only shape we trust. Do not "fix" the
    // badge to live without a live test that proves a mid-match flip takes.
    { n:'modStats',                    lbl:'Player Stats Profile',     type:'sel', def:'0', eff:'restart', opts:[['0','Real BO1 rank (shared)'],['1','Per-mod stats (private)']], tip:'modStats\nWHICH PLAYER PROFILE THIS SERVER READS AND WRITES.\n1 = Plutonium default: every mod gets its OWN stat profile keyed to fs_game (players\mods\mp_gunfight\mpstats). A private level-1 ladder -- rank never carries in from vanilla servers and nothing earned here leaves.\n0 = GF default: use the BO1 COREGAME profile. Players arrive at their real rank and the XP they earn here goes back into it, shared with vanilla and every other unmodded ranked server.\nVERIFIED REAL, not a placebo: a live rcon read returns is:"1" default:"1" Domain is 0 or 1 -- a typed domain and a fixed registered default, the same signature as g_fixBulletDamageDupe and the opposite of the inert g_fix_viewkick_dupe ("Domain is any text").\nTHREE CONSEQUENCES AT 0:\n1. TWO-WAY. Gunfight registers ~5x stock XP (kill 500 vs 100), so real ranks climb ~5x faster than vanilla.\n2. STATS ONLY. Settings and binds live in config_mp.cfg, a separate layer driven by fs_game, so those still namespace per mod -- a player\'s first join still looks like their settings reset.\n3. The _gf_fun account editors (Level 50 / Prestige / Unlock Pro / CoD Points) stop being sandboxed and edit real accounts permanently, with no undo. That is intended here: it is how a player who wants their Gunfight progress carried over gets it restored.\nNot retroactive -- rank already earned in the per-mod profile stays stranded in that file.' },
  ...
  // ⚠ EVERY perk_* dvar is a MAGNITUDE, and the matching specialty_* perk is its GATE. The engine
  // (or _class.gsc) only consults the dvar for a player who HAS the perk — they never "fight", but a
  // slider whose gate nobody holds is a DEAD control that silently does nothing. Each tip therefore
  // states its gate and its SCOPE:
  //   BASE          — every player, every round (the perk is in gf_giveCustomLoadout's base set)
  ...
  { title: 'PERKS — give / remove for all players', via: 'both', vars: [
    { n:'specialty_movefaster',        lbl:'Lightweight',              type:'perk', def:'0', tip:'specialty_movefaster\nFaster movement (+7%, perk_speedMultiplier 1.07). ⚠ OFF by default since 2026-07-16 — the speed half made the tight 42s rounds feel twitchy, so the base set grants only its Pro half (No Fall Damage below). Check to opt everyone back in.' },
    ...
    { n:'specialty_unlimitedsprint',   lbl:'Marathon Pro (Unlimited Sprint)', type:'perk', def:'1', tip:'specialty_unlimitedsprint\nThe sprint meter never empties. BASE perk. This is the engine\'s real Marathon Pro bit (consumed by engine movement code — no stock GSC references it), and it is independent of the scr_gf_sprint_unlimited / player_sprintUnlimited client-dvar path.' },
    { n:'specialty_armorvest',         lbl:'Body Armor (-20% bullet dmg)', type:'perk', def:'1', tip:'specialty_armorvest\n⚠ NOT a Black Ops perk — it is none of the 15. An engine leftover token with no create-a-class row and no icon, but with LIVE damage code: it cuts every NON-HEADSHOT BULLET hit to 80% (perk_armorVest, default 80). BASE perk, kept on purpose for a softer TTK on 42s rounds. Headshots bypass it entirely.' },
    { n:'specialty_flakjacket',        lbl:'Flak Jacket',              type:'perk', def:'1', tip:'specialty_flakjacket\nReduced explosive damage — this is the REAL Flak Jacket. BASE perk. (Its Pro is specialty_fireproof, not granted.)' },
    ...
    { n:'specialty_bulletpenetration', lbl:'Hardened',                 type:'perk', def:'0', tip:'specialty_bulletpenetration\nIncreased bullet penetration through walls and hard surfaces. NOT in the base set — it rides in the sniper/heavy loadout package.' },
    { n:'specialty_bulletflinch',      lbl:'Hardened Pro (less flinch)', type:'perk', def:'0', tip:'specialty_bulletflinch\nReduced reaction and recoil when shot.\n⚠ NOT a base perk, and do not make it one. It gates perk_damageKickReduction (registered default 0.2 = the kick REMAINING, i.e. an 80% cut) which MULTIPLIES with scr_gf_flinch — with it in the base set the live server ran at 10% of stock flinch. It is a SNIPER/HEAVY class trait only.' },
    { n:'specialty_loudenemies',       lbl:'Ninja Pro (louder footsteps)', type:'perk', def:'1', tip:'specialty_loudenemies\nEnemy movement is LOUDER. BASE perk — granted WITHOUT Ninja (specialty_quieter), so nobody is made quieter and everyone hears everyone else louder. It is a listener-side perk, which is what makes giving it to all players symmetric.\n⚠ Engine-consumed (no stock GSC references it) — unverified in-game.' },
    ...
    { n:'specialty_nottargetedbyai',   lbl:'Ghost Pro (no AI targeting)', type:'perk', def:'0', tip:'specialty_nottargetedbyai\nNot targeted by AI killstreaks (dogs, sentries, helis). Half of Ghost Pro — pair with the row below.\nReplaced a "Cold Blooded" row that pushed specialty_blindeye: that token does NOT exist in T5 (MW naming), and SetPerk on an unknown name is a silent no-op, so it never did anything.' },
    { n:'specialty_noname',            lbl:'Ghost Pro (no red name)',  type:'perk', def:'0', tip:'specialty_noname\nNo red name / crosshair change when an enemy targets you. The other half of Ghost Pro.' },
    { n:'specialty_sprintrecovery',    lbl:'Steady Aim Pro (sprint recovery)', type:'perk', def:'0', tip:'specialty_sprintrecovery\nFaster ADS after sprinting. In BO1 this is half of STEADY AIM Pro (shrp.gsc groups bulletaccuracy + sprintrecovery + fastmeleerecovery under PERKS_STEADY_AIM_PRO) — it is not "Extreme Conditioning", which is not a BO1 perk. Pairs with the Sprint Recovery slider.' },
    { n:'specialty_fastmeleerecovery', lbl:'Steady Aim Pro (melee recovery)', type:'perk', def:'1', tip:'specialty_fastmeleerecovery\nFaster recovery after a knife lunge — "recovery rate after lunging with knife is reduced". BASE perk (every player, every round), granted WITHOUT Steady Aim itself.\nThe PERK IS THE WHOLE FEATURE: it gates perk_weapMeleeMultiplier, whose registered default 0.5 already halves recovery time. There is nothing else to set — and 1.0 on that slider would mean NO benefit (its domain is 0.01-1, so 1.0 is the WORST value, not "stock"). Push it toward 0.01 for a near-instant knife.' },
  ...
  { title: 'TEAMS', eff: 'live', per: 'dvar', vars: [
    ...
    // NOT a join blocker (the old label): scr_teambalance is the max team-size difference the stock
    // autobalance loop tolerates, and any non-zero value ARMS that loop. Keep it 0 in Gunfight.
    { n:'scr_teambalance',            lbl:'Stock Autobalance (force-move)',type:'tog',def:'0', tip:'scr_teambalance\nARMS the stock BO1 autobalance loop (_teams.gsc). GF default: 0 (off) — leave it off.\n\nON: every 10s the server checks the roster and, if the teams differ by more than 1, warns "Autobalance in 15 seconds" and then FORCE-MOVES the most recent joiner to the other team. The stock move calls suicide() on them if they are alive — in a one-life gametype that is a player killed mid-round and handed to the enemy.\n\nIt does NOT prevent anyone from joining the larger team (nothing in stock reads this dvar for that).\n\nGunfight balances at ROUND BOUNDARIES instead: the bot fill reconciler (gf_fill_n) and the Balance Teams button, which defers a live player\'s move to the next round.' },
...
const GF_MATCH_VARS = [
    ...
    { n:'scr_gf_timelimit',           lbl:'Round Time — Small (min)', type:'num', def:'0.7',  tip:'scr_gf_timelimit\nRound time for SMALL mode (<=4v4). 0.7 = 42 sec. 0 = no limit.' },
...
const GF_START_VARS = [
    ...
    { n:'scr_gf_load_wait',           lbl:'Load Wait (s, 0=off)',      type:'num', def:'20', tip:'scr_gf_load_wait\nMax seconds the match FIRST round holds BEFORE the prematch countdown until every map-loading client is in, so everyone sees the intro/countdown together (and slow loaders are not grace-locked into spectating round 1). A CEILING, not a duration — releases the moment the last loader is off the loading screen. Bots + demo clients excluded. Shows a "Waiting for teams N/M" readout. First-time FastDL downloaders are not fully absorbed. Default 20. 0 = off (no wait; a slow loader may miss the intro / spectate round 1, and Load Grace goes inert). Clamped 0-120.\nSame hold also enforces Min Players above.' },
    { n:'scr_gf_load_expect',         lbl:'Wait for Full Roster',      type:'tog', def:'1', tip:'scr_gf_load_expect\nMake Load Wait wait for the HEADCOUNT the last match ended with, instead of only for whoever happened to connect inside its blind 3s arrival floor. The count is snapshotted at every match end and is one-shot (consumed by the next match start), so it can never leak into a later, unrelated match.\nWHY IT IS NOT JUST "WAIT FOR EVERYONE": the server cannot see a client that has not sent its connect packet yet — there is no engine read for "clients still connecting" — so the only source of a target number is the previous map\'s roster.\nGAIN: with the room accounted for and loaded, the match starts IMMEDIATELY (a 2-player server that both reconnect in 400ms no longer pays the 3s floor), and nobody is left behind because 3s happened to elapse first.\nCOST: a player who LEFT between matches never sends a connect packet, so the count is never reached and the start is held for Roster Wait seconds (default 6) before falling back to normal behaviour. That is the entire penalty, it is paid once, and the next match end re-snapshots the smaller roster.\nRequires Load Wait > 0 (this refines that gate; Load Wait stays the ceiling). An empty / bot-only lobby never waits.' },
    { n:'scr_gf_load_expect_wait',    lbl:'Roster Wait (s)',           type:'num', def:'6', tip:'scr_gf_load_expect_wait\nSeconds "Wait for Full Roster" waits for the missing players before giving up and falling back to today\'s rule (every client it HAS seen is off the loading screen).\nKeep it SHORT. A genuinely returning player reconnects in ~1-3s — they already hold the map and mod.ff — so this only needs to cover that, while a player who left between matches costs you the full value every match start. 6 is a deliberate compromise; raising it past ~10 mostly just taxes leavers.\nCapped by Load Wait (the load ceiling always wins). Clamped 0-60. Inert unless Wait for Full Roster is on.' },
